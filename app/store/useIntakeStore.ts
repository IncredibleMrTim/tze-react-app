...
export const useIntakeStore = create<IntakeStore>()(
  devtools(
    (set) => ({
      ...
      removePoPage: (index) =>
        set((state) => ({
          poPages: state.poPages.filter((_, i) => i !== index),
        })),
      setPartsOnArrivalPhotos: (partsOnArrivalPhotos) =>
        set({ partsOnArrivalPhotos }),
      ...
      removePartsPhoto: (index) =>
        set((state) => ({
          partsOnArrivalPhotos: state.partsOnArrivalPhotos.filter(
            (_, i) => i !== index,
          ),
        })),
