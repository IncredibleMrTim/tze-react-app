...
/**
 * Jig assignments carousel — shows every jig the job has been loaded on,
 * active or cleared, each with its own reference photo. A cleared jig's
 * photo is permanent history, so it keeps showing here even once the
 * physical jig has moved on to a different job's load.
 */
export function JigAssignmentsSection({
  jobId,
}: JigAssignmentsSectionProps) {
  const { jigPhotosForJob } = useJig(jobId)
                    ...
                    <CarouselItem key={jig.id}>
                      <div className="relative w-full border border-gray-200 border-b-0 rounded-t-lg overflow-hidden">
                        ...
                        <div className="absolute top-3 left-3 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded shadow-lg">
                          Jig {idx + 1} of {jigPhotosForJob.length}
                        </div>
                      </div>
