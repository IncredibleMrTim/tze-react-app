...
/**
 * Enter Job form — the intake form for creating or editing a job.
 *
 * Rendered inside the shared job drawer (see IntakeClient) whenever the
 * intake store's showSheet flag is set. All form state lives in the intake
 * store so the form can be pre-populated (e.g. when editing an existing job
 * or applying a PO scan result).
 */
export function EnterJobForm() {
  const {
    closeSheet,
    customer,
    po_number,
