...
import type { IJob, IContact, IPart, ISettings, IItem, IJigAssignment } from "@/interfaces";
import type { TPlating } from "@/types";
import { Overlay } from "@/components/Overlay";
import { CONTACTS } from "@/lib/helpers";
import { scanPODocument } from "@/lib/api";
...
export const IntakeView: React.FC<IntakeViewProps> = ({ settings, jobs, jigA, onSave, onUpdateJob, onDeleteJob, onShowToast }) => {
  ...
  const handleScanPO = async (file: File) => {
    if (!settings.apiKey) {
      onShowToast('No API key — go to Settings and add your Anthropic key first');
      return;
    }
    ...
    try {
      const result = await scanPODocument(file);
      ...
      setScanResult(`✓ Scanned: ${result.customer_name || 'Unknown'} - ${result.parts.length} parts found`);
  ...
  const getFilteredItems = () => {
    ...
    return ITEMS.filter(item => {
      // Match search term
      const matchesTerm = item.code.toLowerCase().includes(term) ||
                          item.desc.toLowerCase().includes(term);

      // Filter by customer if one is selected (not for internal jobs)
      const matchesCustomer = !customer || isInternal ||
  ...
  const formatJobDate = (timestamp: number) => {
    ...
    return date.toLocaleDateString('en-NZ', { day: 'numeric', month: 'short', year: 'numeric' }).toUpperCase();
  };
    ...
    <div className="relative h-full">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
        <div className="text-base text-gray-600">
          On floor: <span className="font-semibold text-gray-900">{jobs.length} jobs</span>
        </div>
        <button
      ...
      {jobs.length === 0 && (
        <div className="text-center py-16">
          ...
          <div className="text-xl font-semibold text-gray-800 mb-2">Shop floor clear</div>
        ...
        <div className="space-y-4">
          {Object.entries(groupJobsByDate()).map(([dateLabel, dateJobs]) => (
            <div key={dateLabel}>
              ...
              {dateJobs.map(job => {
                const hasJig = jigA.some(g => g.jobId === job.id && !g.completedAt);
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          Arrived: {new Date(job.createdAt).toLocaleDateString('en-NZ', {
                        ...
                        {job.partDescription && (
                          <div className="text-sm text-gray-700 italic mt-1">{job.partDescription}</div>
                        )}
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      <span className="text-gray-700">{job.plating === 'gold' ? 'Gold' : 'Silver'}</span>
                      {job.stringsRequired && (
      ...
      {selectedJob && (
        <div className="pb-20">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  setCustomer(CONTACTS.find(c => c.name === selectedJob.customer_name) || null);
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-4">
            ...
            <div className="text-base text-gray-700 mb-1">{selectedJob.customer_name}</div>
          ...
          {selectedJob.parts.length > 0 && (
            <div className="mb-4">
              ...
              {selectedJob.parts.map((part, idx) => (
                <div key={idx} className="bg-gray-50 rounded-lg p-3 mb-2 flex justify-between items-center">
                  <div>
                    <div className="font-medium text-sm text-gray-900">{part.desc}</div>
                    <div className="text-xs text-gray-500">{part.code}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-semibold text-gray-900">×{part.qty}</div>
                    <div className="text-xs text-gray-600">${part.price.toFixed(2)}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {selectedJob.partsPic && (
            <div className="mb-4">
              ...
              <img src={selectedJob.partsPic} alt="Parts on arrival" className="w-full rounded-lg" />
            </div>
          )}
          ...
          {selectedJob.partDescription && (
            <div className="mb-4">
              ...
              <div className="text-sm text-gray-700">{selectedJob.partDescription}</div>
            </div>
          )}
          ...
          <div className="mb-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-sm text-gray-900">Arrived</div>
          ...
          <div className="px-4 pt-5">
            ...
            <div className="mb-5">
              <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1 block">
                SCAN PO — <span className="font-normal normal-case text-gray-400">Claude auto-fills the form</span>
              </label>
              <input
                ref={fileInputRef}
                type="file"
              ...
              {scanResult && (
                <div className={`text-sm mt-2 px-3 py-2 rounded ${
                  scanResult.startsWith('✓') ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
                }`}>
                  {scanResult}
                </div>
              )}
            ...
            <div className="mb-4">
              <label className="text-[13px] font-medium text-gray-700 mb-1.5 block">
                PO number <span className="text-gray-400 font-normal text-xs">(leave blank to auto-assign)</span>
              </label>
              <input
                type="text"
                value={po_number}
            ...
            <div className="mb-5">
              <label className="text-[13px] font-medium text-gray-700 mb-1.5 block">
                Parts description <span className="text-gray-400 font-normal text-xs">(optional — shows on job card)</span>
              </label>
              <input
                type="text"
            ...
            <div className="mb-5">
              <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2 block">
                JIG RATE OPTIONS — <span className="font-normal normal-case text-gray-400">fill in if no part prices on PO</span>
              </label>

              <div className="space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">Strings required</div>
                    <div className="text-xs text-gray-500">JIG cannot complete until string count is entered</div>
                  </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">Requires weighing</div>
                    <div className="text-xs text-gray-500">Dispatch blocked until weight is entered</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
              ...
              {parts.map((part, i) => (
                <div key={i} className="border border-gray-200 rounded-lg p-3 mb-3 bg-gray-50">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs text-gray-500 uppercase">Part {i + 1}</span>
                    <button
                    ...
                    {partSearchIndex === i && getFilteredItems().length > 0 && (
                      <div className="absolute top-full left-0 right-0 bg-white border-2 border-primary border-t-0 rounded-b-lg max-h-[300px] overflow-y-auto z-[500] shadow-lg">
                        {getFilteredItems().map((item, idx) => (
                          ...
                          >
                            <div className="font-semibold text-sm text-gray-900">{item.code}</div>
                            <div className="text-xs text-gray-600">{item.desc} — ${item.price.toFixed(2)}</div>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-xs text-gray-600 mb-1 block">Price per part ($)</label>
                      <input
                        type="number"
                        value={part.price}
            ...
            <div className="mb-5">
              <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2 block">
                PARTS / ARRIVAL PHOTO <span className="text-gray-400 font-normal normal-case">(optional)</span>
              </label>
              <input
              ...
              {!partsPic ? (
                ...
                >
                  ...
                  <div className="font-medium text-gray-700">Tap to photograph parts on arrival</div>
                  <div className="text-xs text-gray-500">Condition record — pallet, box or loose parts</div>
                </button>
              ) : (
                <div className="relative">
                  <img src={partsPic} alt="Parts photo" className="w-full rounded-lg border-2 border-gray-300" />
                  <button
            ...
            <div className="space-y-3 mb-5">
              ...
              <div className="border-2 border-red-300 rounded-lg p-3 bg-red-50">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-red-600">Highlighted red — jumps the queue</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
              ...
              <div className="border border-gray-300 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">🏭 Internal TGAEP</div>
                    <div className="text-xs text-gray-500">Internal job — no invoice at dispatch</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
              ...
              <div className="border border-gray-300 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">Freight requested</div>
                    <div className="text-xs text-gray-500">Customer requested freight — cost added at dispatch</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
              ...
              <div className="border border-gray-300 rounded-lg p-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-900">Minimum charge</div>
                    <div className="text-xs text-gray-500">Apply $60.00 minimum (Silver)</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
              ...
              <div className="border-2 border-orange-300 rounded-lg p-3 bg-orange-50">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-orange-700">🔧 Flag this job</div>
                    <div className="text-xs text-orange-600">Mark for follow-up — price correction, missing Xero item, etc</div>
                  </div>
                  <label className="relative inline-block w-12 h-6 ml-3">
                    <input
                      type="checkbox"
